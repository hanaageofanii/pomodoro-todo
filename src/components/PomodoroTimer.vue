...
// Terbawa ke document title saat di environment browser
...
// Memastikan Web Audio aman diakses jika server-side rendering digunakan
...
/* --- PERBAIKAN TOTAL BOX TIMER AGAR RESPONSIVE --- */
...
/* Solusi Kotak Hitam Meluber: Menggunakan clamp() & width fleksibel */
.time-display { 
...
/* --- TO-DO LIST (RESPONSIVE INPUT) --- */
.todo-input-row { display: flex; gap: 6px; margin-bottom: 0.75rem; width: 100%; }
.todo-input { flex: 1; min-width: 0; background: var(--surface2); border: 1px solid var(--border); color: var(--text); border-radius: var(--radius); padding: 8px 12px; font-size: 13px; outline: none; transition: border 0.15s; }
.todo-input:focus { border-color: var(--accent2); }
.todo-input::placeholder { color: var(--muted); }
.btn-add { background: var(--accent2); color: #111; border: none; border-radius: var(--radius); padding: 8px 14px; font-size: 18px; cursor: pointer; font-weight: bold; flex-shrink: 0; }

.todo-list { list-style: none; display: flex; flex-direction: column; gap: 5px; max-height: 220px; overflow-y: auto; }
.todo-item { display: flex; align-items: center; gap: 8px; background: var(--surface2); border: 1px solid var(--border); border-radius: var(--radius); padding: 8px 10px; width: 100%; }
.todo-item.done { opacity: 0.5; }
.todo-cb { width: 17px; height: 17px; accent-color: var(--accent); cursor: pointer; flex-shrink: 0; }
.todo-text { flex: 1; min-width: 0; font-size: 13px; cursor: pointer; word-break: break-word; }
.todo-item.done .todo-text { text-decoration: line-through; color: var(--muted); }
.todo-del { background: none; border: none; color: var(--muted); font-size: 15px; cursor: pointer; padding: 0 2px; flex-shrink: 0; }
...
/* --- AMBIENT SOUND --- */
.ambient-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px; margin-bottom: 0.75rem; width: 100%; }
